...
echo "  2. Apply ArgoCD app:  kubectl --kubeconfig $KUBECONFIG_PATH apply -f argocd/apps/observability/monitoring-secrets.yaml"
echo "  3. Update minio + loki + prometheus-stack helm values to use existingSecret"
echo "  4. git add $OUT_DIR argocd/apps/observability/monitoring-secrets.yaml && git commit && git push"
